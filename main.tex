\documentclass[]{main}

\begin{document}
\lastupdated

%%%%%%%%%%%%%%%%%%          TITLE NAME      %%%%%%%%%%%%%%%%%%
\namesection{UMANG J}{GALA} { - TECHNICAL ARCHITECT}
{\href{mailto:toumangg@gmail.com}{toumangg@gmail.com}  |  +91 . 99098 . 79929  | 
LinkedIn: \href{https://www.linkedin.com/in/iumangg}{\custombold{iumangg}} | Github: \href{https://github.com/iumangg}{\custombold{iumangg}}

}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     PAGE 1: COLUMN ONE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{minipage}[t]{0.25\textwidth} 

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %     LINKS
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% \section{Links} 
% Github:// \href{https://github.com/galaumang}{\custombold{galaumang}} \\
% LinkedIn://  \href{https://www.linkedin.com/in/galaumang}{\custombold{galaumang}} \\
% \sectionsep\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SKILLS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Skills}
\subsection{Programming}\sectionsep
\textbullet{} Java \textbullet{} Typescript \textbullet{} Javascript\\
\textbullet{} Python \textbullet{} C/C++ \textbullet{} PHP\\
\textbullet{} HTML 5 \textbullet{} CSS\\
\textbullet{} SQL \textbullet{} Shell / Bash

\sectionsep
\subsection{Framework \& Libraries}\sectionsep
\textbullet{} React \textbullet{} Angular\\
\textbullet{} Kafka \textbullet{} Zookeeper\\
\textbullet{} Spark - Stream / SQL \textbullet{} Hadoop\\
\textbullet{} Avro \textbullet{} Protobuf\\
\textbullet{} Spring \textbullet{} PJ 2 \textbullet{} JUnit
\sectionsep

\subsection{Platform}\sectionsep
\textbullet{}  Azure - AKS, Storage, EventHub, Networking, etc\\
\textbullet{}  AWS - EC2, Lambda, S3, DDB, SQS, SNS, Cloudwatch, etc\\
\textbullet{}  Kubernetes \& Docker
    % Cloudera & Android
\sectionsep

\subsection{Software Tools}\sectionsep
\textbullet{} Node.js \textbullet{} Gradle \textbullet{} Maven\\
\textbullet{} AWS CLI \textbullet{} Jenkins\\
\textbullet{} GIT  \textbullet{} GitHub\\ 
\textbullet{} VS Code \textbullet{} IntelliJ Idea 
    % Oracle, Weka & R
\sectionsep

\subsection{OS}\sectionsep
\textbullet{} Mac OS \textbullet{} Windows \textbullet{} Linux  
    % Rasbian OS
\sectionsep

\subsection{Soft Skills}\sectionsep
Leadership\\
% Software Designing\\
Project Management\\
Team Management\\
Problem-Solving
\sectionsep

\section{Certification}
% \sectionsep
\subsection{Cloud}
\sectionsep
AWS Cloud Practitioner\\
Microsoft Azure Fundamentals\\
% \sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EDUCATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Education} 
\subsection{Rochester Institute of Technology}
\sectionsep
\descript{M.S. in CS}
\location{CGPA - 3.29}
\location{May 2015 | Rochester, NY}
\sectionsep\sectionsep

\subsection{K J Somaiya College of Engineering}
\sectionsep
\descript{B.E. in IT}
\location{Aggregate - 66\%}
\location{May 2013 | Mumbai, India}
\sectionsep\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN TWO
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{minipage} 
\hfill
\begin{minipage}[t]{0.73\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EXPERIENCE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Experience}

\runsubsection{Microsoft}
\descript{| Senior Software Engineer | Technical Architect }
\location{Nov 2024 - Present | Noida, India}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item \custombold{Core Responsibilities \& Ownership}: Architected and delivered high-scale backend services for Microsoft Purview Data Map, ensuring resilient ingestion pipelines, optimized metadata storage, and consistent performance at enterprise scale. 
\item \custombold{Technical Impact}: Drove deep technical improvements in distributed backend systems, eliminating scalability bottlenecks and strengthening reliability for high-throughput enterprise metadata services
\item \custombold{Customer \& Business Value}: Delivered customer-centric backend features for Microsoft Purview Data Map, translating enterprise governance needs into reliable, scalable capabilities that improved metadata discoverability and trust.
\item \custombold{Cross-Team Collaboration}: Drove cross-team collaboration across scanning, catalog, and MIP/RBAC teams, working closely with PM, design, and partner engineers to translate customer requirements into scalable, production-ready solutions across the Purview ecosystem.
\item \custombold{Security, Quality \& Compliance}: Owned SFI-driven quality and security improvements for the service, strengthening CI/CD, test coverage, and SDL compliance to meet Microsoft engineering standards.
\item \custombold{Leadership \& Mentorship}: Provided technical mentorship to junior engineers, driving best practices in system design and distributed systems, and fostering a culture of engineering excellence through documentation, knowledge sharing, and post-incident learning.
partnered with JP Morgan
\end{tightemize}
\sectionsep

\runsubsection{Amazon}
\descript{| Technical Architect \& Designer }
\location{May 2021 - Feb 2023 | Hyderabad, India}
\begin{tightemize}
\item Architected an end-to-end data labeling platform used by 4 engineering teams, enabling ML scientists to train AI models with image, video, and text data. (Java, Python, React, AWS)
\item Designed a secure microservice architecture to protect Alexa customer data; this privacy-enforcing pattern was mandated and adopted as the standard across all org-wide engineering teams. (Java, AWS)
\item Developed a modular UI framework with a plugin system adopted by 3 teams, streamlining development and component ownership in a large-scale web application. (React, HTML, CSS)
\item Partnered with the AWS organization to co-develop and influence the roadmap for core machine learning platform features, impacting both internal and external services.
\item Provided technical mentorship to new engineers, accelerating their integration and driving best practices in system design and code quality.
\item Championed Agile methodologies and upheld strict SDLC standards, ensuring all projects adhered to rigorous security, privacy, and IT controls.
% \item Designed a customer-facing system that supports data labelling for ML scientists to train and develop their AI models, collaborated with 4 different engineering teams to implement. The system supports end-to-end flow from requesting labelling to delivery of quality labelled data.  (Java, Python, React, AWS services like EC2 / Lambda / SQS / DDB etc)
% \item Designed microservice-based backend architecture to enable secure access to Alexa'''s data from the internal data warehouse tool protecting customer privacy, this architecture was mandated in all the services across org'''s engineering teams (Java, Javascript, AWS - API Gateway / EC2 / Lambda / SQS / DDB)
% \item Designed and implemented a common UI framework for integrating custom plugins to the UI platforms, which indeed was used by 3 engineering teams to efficiently manage the ownership of UI components in a single web application (React, HTML, CSS)
% \item Designed and developed a feature within the data labelling system to support photo/video data labelling for specific ML scientists requiring quality labelled data for customer visual data such as photos and videos. (Java, React)
% \item Designed and developed a feature within the data labelling system to support photo/video data labelling that helped developed ML models on visual data. (Java, React)
% \item Collaborated with AWS organisation to evolve the AWS service offerings for ML scientists internally as well as externally by designing a vast range of data labelling features and improving the ML model execution service.
% \item Collectively working with the AWS team to evolve the AWS offerings for internal and external customers by designing and implementing core platform features for ML scientists (Java, Javascript, React, AWS)
% \item Mastered software engineer using Agile methodology by participating in product backlog grooming, sprint planning, review and retrospective
% \item Volunteered as a mentor for new software engineers in their transition to the team/firm \& help develop their professional career
\end{tightemize}
\sectionsep



\runsubsection{JP Morgan Chase \& Co.}
\descript{| Senior Software Engineer }
\location{Feb 2019 - May 2021 | Newark, DE}
\begin{tightemize}
\item Designed and developed a distributed data lake application purely driven by metadata for easy usability and scalability (Kubernetes, Spark, Java, Kafka, Avro, Protobuf)
\item Designed and developed a Real-Time Model Transformation application that converts any input data model to a specified output model with data lineage (Spark Stream/SQL, Java, Avro) 
\item Development and test distributed application changes using disciplined software development processes (Java, Kafka, Zookeeper, Avro)
\item Day-to-day maintenance of the application system in operations, including tasks related to identifying and troubleshooting application issues and issues resolution or escalation and catering to ad-hoc work requests
\item Adherence to Development Life Cycle, IT controls and change control policies for all developments
\item Mastered software engineer using Agile methodology by participating in product backlog grooming, sprint planning, review and retrospective
%\item Mentor new software engineers in their transition to the team/firm \& help develop their professional career
\end{tightemize}
\sectionsep

\end{minipage} 

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     PAGE 2: COLUMN ONE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{minipage}[t]{0.25\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     COURSEWORK
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Coursework}
% \sectionsep

\subsection{Graduate}
\sectionsep
Parallel Computing\\
Data Security \& Privacy\\
Distributed Systems\\
Computer Networks\\
Cryptography\\
Neural Networks\\
Machine Learning\\
Big Data\\
Algorithms\\
\sectionsep

\subsection{Undergraduate}
\sectionsep
Information \& Network Security\\
Software Engineering\\
Object Oriented Programming\\
Data Structures\\
Database Technologies\\
Microcontrollers and processors\\
\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     Interest
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Personal Interests}\sectionsep
Reading\\
Writing\\
Travelling\\
Volunteering\\
Photography\\

% \section{Presentations}

% \subsection{Guitar Distortion using VST plugins} 
% \sectionsep
% shortlisted for final round of International Student Design competition at Audio Engineering Society Convention, New York City, 2015.

\end{minipage} 
\hfill
\begin{minipage}[t]{0.73\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     PROJECTS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Experience}

\runsubsection{JP Morgan Chase \& Co.}
\descript{| Software Engineer }
\location{Feb 2015 - Jan 2019 | Newark, DE}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Developed full-stack features using Java (Spring, REST) and Angular, including a metadata-driven data browser and a scalable, configuration-based data landing framework.
\item Improved system security and insights by building an automated vulnerability scanner (Bash) and developing various data reports using ETL tools (Pentaho, Oracle).
\item Demonstrated leadership by mentoring peers to promote code reuse and volunteering as Tech Lead to successfully deliver a web application for a non-profit partner.
% \item Developed and tested demo examples for the beta release of a new upcoming System Toolbox. 
% \item Developed metadata-driven web application for data browsing (Angular, Java Rest API)
% \item Developed application framework for data landing, scalable by configuration
% \item Developing a new feature to the application using Java Spring framework (Java, Spring, MQ, Pentaho)
% \item Developed an automated script to scan the application repository for any vulnerabilities (Bash, Unix)
% \item Responsible to conduct knowledge sharing sessions within/outside the team to increase code reusability
% \item Developed various reports using the Pentaho ETL tool (Java, Pentaho, Oracle)
% \item Volunteered as Tech Lead to develop and successfully deliver a web application for a non-profit organization partnered with JP Morgan
\end{tightemize}
\sectionsep

\runsubsection{JP Morgan Chase \& Co.}
\descript{| Software Engineer - Intern }
\location{Jun 2014 - Aug 2014 | Newark, DE}
\begin{tightemize}
\item Developed a full-stack back-office module (Java, Spring, ExtJs) and optimized a data processing program to handle hundreds of thousands of entries efficiently.
\item Built an automated testing infrastructure using Selenium, including automatic test report generation and maintenance of the virtual testing environment (Unix).
% \item Developed a module for the Back office application using ExtJs and Java Spring framework
% \item Developed a program that processes hundreds of thousands of entries in more efficient way to increase the performance (Java, SQL)
% \item Developed a automated testing infrastructure for the application using Selenium Java API
% \item Developed a program to generate test reports automatically (Java)
% \item Responsible for maintaining and upgrading the virtual environment for application testing (Unix)
\end{tightemize}
\sectionsep

\runsubsection{Rochester Institute of Technology}
\descript{| GA - System Admin, CS Department }
\location{Aug 2014 - Apr 2015 | Rochester, NY}
\begin{tightemize}
\item Managed CS department infrastructure, including server/lab maintenance, user account administration, and Hadoop cluster setup; provided real-time technical support and developed automation scripts (Unix, Bash) to monitor resources.
% \item Responsible to setup Hadoop cluster environment in the lab
% \item Responsible to maintain/upgrade CS department servers, lab machine and user accounts
% \item Developed a script to automate used disk space calculation for faculty/user accounts (Unix, bash, C shell)
% \item Responsible for solving/supporting real time technical problems faced by faculties, staffs and students
\end{tightemize}
\sectionsep

\section{Projects}
\sectionsep

\runsubsection{Financial Analysis Engine} \location{Sep 2025 - Present} 
Designed and developing an intelligent MCP server to answer complex financial queries by orchestrating multiple data models, seamlessly integrating historical and real-time data to perform unified analytics.
\sectionsep

\runsubsection{Scalable Generic Data Lake Platform} \location{Oct 2024 - Present} 
Architected a highly scalable, metadata-driven data lake platform capable of serving diverse domains such as finance, e-learning, and content management, establishing a generic framework for rapid data integration and analytics.
\sectionsep

\runsubsection{Indoor Localization} \location{Aug 2014 - May 2015}
A mobile application that locate an object or a people inside a building using Received Signal Strength (RSS) from already deployed WiFi access point, as GPS is not suitable for indoor condition.
It also help getting direction in huge indoor environments like schools, malls, airports, etc.
Technologies used Android, Java \& C++
\sectionsep

\runsubsection{Hybrid Parallel Logistic Regression} \location{Sep 2014 - Nov 2014}
Used the Parallel Java 2 library to implement parallel logistic regression for machine learning.
Ran experiments on a 10-node cluster parallel computer using very large and very sparse data-sets (3 million records against 2 million features).
Technologies used Java, PJ2 Java library
\sectionsep

\runsubsection{Stock Recommendation Model} \location{Feb 2014 - Apr 2014}
A system that recommends the stocks to users based on the current stocks in their portfolio. 
The system uses k means clustering algorithm on historical NASDAQ data to cluster the stocks. 
Technologies used MySQL database, SQL, Weka \& PHP
\sectionsep

\runsubsection{Real-Time Editing System} \location{Sep 2012 - Apr 2013}
A groupware system that allows group of users to view and edit the one document at real time. 
A user can create or upload a document and invite other users to contribute to that document. 
It provides access privileges settings to the owner of the document.
Technologies used PHP, HTML, CSS, JavaScript, JQuery, Node.js \& SQL
\sectionsep

\runsubsection{Information Sharing} \location{Aug 2014 - May 2015}
A web application where users can create groups and share information or documents with individual or groups.
It has a real time chat room to conduct online meetings.
Technologies used JSP, Servlet, Javascript, HTML, CSS and SQL
\sectionsep

\runsubsection{Trivial File Transfer Protocol} \location{Aug 2014 - Nov 2014}
Implemented TFTP used to transfer files that runs on top of UDP. This TFTP protocol is based on RFC-1350.
Technology used Java
\sectionsep

\end{minipage}

\end{document}  \documentclass[]{article}
